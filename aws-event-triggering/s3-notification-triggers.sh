...
role_response=$(aws iam create-role --role-name s3-sns --assume-role-policy-document '{
  "Version": "2012-10-17",
  "Statement": [{
...
zip -r s3-function.zip ./s3-function
...
aws lambda create-function \
  ...
  --runtime "python3.8" \
  --handler "s3-function/s3-function.lambda_handler" \
  --memory-size 128 \
  --timeout 30 \
  --role "arn:aws:iam::$aws_account_id:role/$role_name" \
  --zip-file "fileb://./s3-function.zip"
...
LambdaFunctionArn="arn:aws:lambda:us-east-1:$aws_account_id:function:s3-function"
